...
function Outside.CloudWatch()
      ...
      UI:SetSpeaker(maru)
      UI:SetSpeakerEmotion("Normal")
      UI:WaitShowDialogue("Maybe we should just go,[script=0] mom promised some pie would be waiting on us by now.", {turn})
      ...
      SV.Story.chap = -1
      local talk_evt1 = RogueEssence.Dungeon.BattleScriptEvent("MaruInteract")
            _DATA.Save.ActiveTeam.Players[0].ActionEvents:Add(talk_evt1)
      local talk_evt2 = RogueEssence.Dungeon.BattleScriptEvent("AzuraInteract")
            _DATA.Save.ActiveTeam.Players[1].ActionEvents:Add(talk_evt2)
end
